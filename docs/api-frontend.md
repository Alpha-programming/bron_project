...
Авторизация не обязательна. Если токен есть — передавайте его, тогда просмотр привязывается к пользователю, а не к IP.
...
```json
{"counted": true, "views_count": 15}
```

- `counted: true` — просмотр засчитан, `false` — уже учтён ранее (или это владелец бизнеса).
- Правило уникальности: один пользователь (по токену, без токена — по IP) даёт +1 не чаще **одного раза в 24 часа** на бизнес. Владелец свой бизнес не накручивает.
- `views_count` в ответе всегда актуальный, можно сразу показывать.
- `404` — бизнеса нет.
